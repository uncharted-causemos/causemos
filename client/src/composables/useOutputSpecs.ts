...
export default function useOutputSpecs(
  ...
  const outputSpecs = computed<OutputSpecWithId[]>(() => {
    ...
    return selectedTimeseriesPoints.value
      .map((timeseriesInfo, indx) => {
        const { timeseriesId, scenarioId, timestamp } = timeseriesInfo;
        const featureInfo =
          (breakdownOption?.value === SPLIT_BY_VARIABLE
            ? activeFeatures.value[indx]
            : activeFeatures.value.find((f) => f.name === activeFeature?.value)) ??
          activeFeatures.value[indx];
        const transform =
          featureInfo.transform !== DataTransform.None ? featureInfo.transform : undefined;
        const featureName =
          breakdownOption?.value === SPLIT_BY_VARIABLE
            ? timeseriesId
            : activeFeature?.value ?? activeFeatures.value[indx].name;
        ...
        if (breakdownOption?.value === SPLIT_BY_VARIABLE) {
          if (!temporalResolution || !temporalAggregation || !spatialAggregation) {
            ...
            if (
              ...
              feature = activeFeatures.value.find(
                (f) => f.temporalResolution && f.temporalAggregation && f.spatialAggregation
              );
            }
        ...
        const runId =
          breakdownOption?.value === SPLIT_BY_VARIABLE && scenarioId.endsWith(featureName)
