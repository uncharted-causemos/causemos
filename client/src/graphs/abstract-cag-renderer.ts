...
const AMBIGUOUS_MSG =
  'To make CAG projections easier to interpret, <br /> select each grey edge and clarify its polarity in the side panel.';
...
export abstract class AbstractCAGRenderer<V, E> extends DeltaRenderer<V, E> {
  ...
  displayGraphStats() {
    ...
    if (hasAmbiguousEdges) {
      statsGroup
        .select('.graph-stats-ambiguouity')
        .text('Ambiguous edges detected')
        .on('mouseover', (evt) => {
  ...
  neighborhoodAnnotation({ nodes, edges }: { nodes: NeighborNode[]; edges: NeighborEdge[] }) {
    ...
    const nonNeighborEdges = chart
      .selectAll('.edge')
      .filter(
        (d: any) =>
          !_.some(edges, (edge) => edge.source === d.data.source && edge.target === d.data.target)
      );
  ...
  resetAnnotations() {
    ...
    chart
      .selectAll('.node-container, .node-container-outer') // Clean up previous highlights
  ...
  selectEdgeBySourceTarget(source: string, target: string, color: string) {
    const edge = this.chart
      .selectAll('.edge')
      .filter((edge: any) => edge.source === source && edge.target === target);
    if (edge) {
  ...
  applyVisualState(visualState: CAGVisualState) {
    ...
    if (visualState.outline.nodes) {
      ...
      for (const edge of visualState.outline.edges) {
        this.selectEdgeBySourceTarget(
          edge.source,
          edge.target,
          edge.color ? edge.color : SELECTED_COLOR
        );
      }
    }
  }
  ...
  // Create a contour
  bubbleSet(
    { bubbleNodes, bubbleEdges }: { bubbleNodes: NeighborNode[]; bubbleEdges: any[] },
    color = '#369'
  ) {
    const flattened = flattenGraph(this.graph);
...
export { moveTo, highlight, unHighlight };
