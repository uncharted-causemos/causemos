...
import { RawOutputDataPoint, RawOutputGeoJson } from '@/types/Outputdata';
...
export const filterRawDataByRegionIds = (
  data: RawOutputDataPoint[],
  regionIds: string[]
): RawOutputDataPoint[] => {
  ...
  return data.filter((d) => {
    ...
    const matches = adminLevelsList.map((adminLevels) => {
      ...
      const checks = [
        d.country === adminLevels[0],
        d.admin1 === adminLevels[1],
        d.admin2 === adminLevels[2],
        d.admin3 === adminLevels[3],
      ];
...
export const computeTimeseriesFromRawData = (
  data: RawOutputDataPoint[],
  ...
  const timeseries = Object.values(dataByTs).map((dataPoints) => {
    ...
    return {
      ...
      value: aggregation === AggregationOption.Sum ? sum : sum / dataPoints.length,
    };
  });
