    ...
    <div v-else class="scenarios-container">
      ...
      >
        ...
        <div class="checklist-item-scenario">
          <div class="checklist-item-scenario-title-and-icon" @click="selectScenario(scenario.id)">
            <i v-if="selectedScenarioId === scenario.id" class="fa fa-circle" />
            <i v-else class="fa fa-circle-o" />
            <div
              class="scenario-title"
              :class="{
                'scenario-title-stale': isFullScenario(scenario) && !scenario.is_valid,
              }"
            >
              {{
                scenario.name + (isFullScenario(scenario) && !scenario.is_valid ? ' (Stale)' : '')
              }}
            </div>
          </div>
          <options-button
          ...
          >
            <template #content>
              ...
              <div class="dropdown-option" @click="duplicateScenario(scenario)">
        ...
        <message-display
          v-if="
            isFullScenario(scenario) &&
            !scenario.is_baseline &&
            getScenarioClamps(scenario).length === 0
          "
        ...
        <div v-if="isFullScenario(scenario)">
          ...
          >
            ...
            <div class="scenario-clamp-name">
              {{ ontologyFormatter(clamp.concept) }}
            </div>
            <i
              class="fa fa-fw fa-close delete-scenario-clamp"
...
export default defineComponent({
  ...
  methods: {
    ...
    isFullScenario(scenario: Scenario | HistoricalOnlyScenario): scenario is Scenario {
