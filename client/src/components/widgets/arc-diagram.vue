...
export default {
  ...
  methods: {
    ...
    renderNodes() {
      ...
      // Draw nodes
      const node = chart.selectAll('.nodes')
        .data(nodes)
        .enter()
        .append('g')
        .attr('class', 'nodes');
    ...
    renderEdges() {
      ...
      const xCenter = 0.5 * width;
      const xOffset = 2.0;
      const xRadiusScale = d3.scaleLinear().domain([0, height]).range([10, (0.5 * width - xOffset)]);
    ...
    clearHighlights() {
      const svg = d3.select(this.$refs.container);
      this.chart.selectAll('.nodes path')
        .style('stroke', DEFAULT_STYLE.node.stroke)
        .style('stroke-width', DEFAULT_STYLE.node.strokeWidth);

      this.chart.selectAll('.nodes')
        .style('opacity', 1);

      this.chart.selectAll('.edges').style('opacity', 1).style('stroke', (d) => {
    ...
    highlightSubgraph() {
      ...
      if (!_.isEmpty(this.highlights.selectedNode)) {
        this.chart.selectAll('.nodes path')
          .filter(d => d.concept === this.highlights.selectedNode && modelId === this.highlights.modelId)
    ...
    checkPresentEdge(arrayOfEdges, edge) {
      ...
      const match = arrayOfEdges.filter(e => e.source === edge.source && e.target === edge.target);
      return match[0];
    }
  }
};
