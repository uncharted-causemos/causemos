        ...
        <template #controls>
          <i
            ...
            :class="{
              ...
              'fa-square-o': !relationshipGroup.meta.checked && !relationshipGroup.isSomeChildChecked,
              'fa-minus-square-o': !relationshipGroup.meta.checked && relationshipGroup.meta.isSomeChildChecked
            }"
...
export default {
  ...
  methods: {
    ...
    refresh() {
      ...
      const missingCauseEdges = this.causeEdges.filter(edge =>
        _.findIndex(causeStatement, s => s.subj.concept === edge.source) === -1);
      const missingEffectEdges = this.effectEdges.filter(edge =>
        _.findIndex(effectStatement, s => s.obj.concept === edge.target) === -1);
