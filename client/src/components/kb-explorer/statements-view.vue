    ...
    <template v-if="statements.length > 0">
      <div class="table-container flex-grow-1 h-0">
        <table class="table">
          <thead>
            <tr>
              <th v-if="numContextVisible > 0" :colspan="numContextVisible">Context</th>
              <th :colspan="numSubjVisible">Cause</th>
              <th v-if="numRelVisible > 0" :colspan="numRelVisible">Relation</th>
              <th :colspan="numObjVisible">Effect</th>
              <th v-if="numEvidenceVisible > 0" :colspan="numEvidenceVisible">Evidence</th>
              <th v-if="visibleColumns[FIELDS.READERS]" colspan="1">Reader</th>
            </tr>
            <tr>
              <th v-if="visibleColumns[FIELDS.YEAR]">Temporal Context - Year</th>
              <th v-if="visibleColumns[FIELDS.GEO_LOCATION_NAME]">Geospatial Context</th>
              <th v-if="visibleColumns[FIELDS.SUBJ_POLARITY]">
                Polarity <sort-indicator :field="FIELDS.SUBJ_POLARITY" />
              </th>
              <th v-if="visibleColumns[FIELDS.SUBJ_ADJECTIVES]">Adjectives</th>
              <th v-if="visibleColumns[FIELDS.SUBJ]">Name</th>
              <th v-if="visibleColumns[FIELDS.SUBJ_CONCEPT]" class="highlight">Concept</th>
              ...
              <th v-if="visibleColumns[FIELDS.OBJ_ADJECTIVES]">Adjectives</th>
              <th v-if="visibleColumns[FIELDS.OBJ]">Name</th>
              <th v-if="visibleColumns[FIELDS.OBJ_CONCEPT]" class="highlight">Concept</th>
          ...
          <tbody>
            <tr v-for="statement of statements" :key="statement.id">
              <td v-if="visibleColumns[FIELDS.YEAR]">{{ statement.years.join(', ') }}</td>
              <td v-if="visibleColumns[FIELDS.GEO_LOCATION_NAME]">
                {{ locationFormatter([statement.subj.geo_context, statement.obj.geo_context]) }}
              </td>
              <td v-if="visibleColumns[FIELDS.SUBJ_POLARITY]">
                {{ polarityFormatter(statement.subj.polarity) }}
              </td>
              <td v-if="visibleColumns[FIELDS.SUBJ_ADJECTIVES]">
                {{ listFormatter(statement.subj.adjectives) }}
              </td>
              <td v-if="visibleColumns[FIELDS.SUBJ]">{{ statement.subj.factor }}</td>
              <td v-if="visibleColumns[FIELDS.SUBJ_CONCEPT]" class="highlight">
                {{ ontologyFormatter(statement.subj.concept) }}
              </td>
              <td v-if="visibleColumns[FIELDS.SUBJ_CONCEPT_SCORE]">
                {{ precisionFormatter(statement.subj.concept_score) }}
              </td>

              <td v-if="visibleColumns[FIELDS.TYPE]">{{ statement.type }}</td>
              <td v-if="visibleColumns[FIELDS.STATEMENT_POLARITY]">
                {{ statementPolarityFormatter(statement.wm.statement_polarity) }}
              </td>
              <td v-if="visibleColumns[FIELDS.BELIEF]">
                {{ precisionFormatter(statement.belief) }}
              </td>

              <td v-if="visibleColumns[FIELDS.OBJ_POLARITY]">
                {{ polarityFormatter(statement.obj.polarity) }}
              </td>
              <td v-if="visibleColumns[FIELDS.OBJ_ADJECTIVES]">
                {{ listFormatter(statement.obj.adjectives) }}
              </td>
              <td v-if="visibleColumns[FIELDS.OBJ]">{{ statement.obj.factor }}</td>
              <td v-if="visibleColumns[FIELDS.OBJ_CONCEPT]" class="highlight">
                {{ ontologyFormatter(statement.obj.concept) }}
              </td>
              <td v-if="visibleColumns[FIELDS.OBJ_CONCEPT_SCORE]">
                {{ precisionFormatter(statement.obj.concept_score) }}
              </td>

              <td v-if="visibleColumns[FIELDS.NUM_EVIDENCES]">{{ statement.wm.num_evidence }}</td>
              <td v-if="visibleColumns[FIELDS.SENTENCES]" class="sentences">
      ...
      <pagination :label="'statements'" :total="statementsCount" />
    </template>
    <message-display v-if="statements.length === 0" message="Sorry, no results were found" />
  </div>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    refresh() {
      ...
      projectService
        .getProjectStatements(this.project, this.filters, {
          from: this.pageStart,
          size: this.pageLimit,
          sort: this.sort,
        })
