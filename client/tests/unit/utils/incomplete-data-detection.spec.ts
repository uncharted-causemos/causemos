...
describe('incomplete-data-detection', () => {
  it('returns NotRequired for mean agg', () => {
    const timeseries: TimeseriesPoint[] = [{ timestamp: 0, value: 10 }];
    const result = correctIncompleteTimeseries(
      timeseries,
  ...
  it('returns OutOfScopeData for invalid last raw date', () => {
    const timeseries: TimeseriesPoint[] = [{ timestamp: MAR_1970, value: 10 }];
    const result = correctIncompleteTimeseries(
      timeseries,
      TemporalResolution.Monthly,
      TemporalResolutionOption.Year,
  ...
  it('returns OutOfScopeData for different years', () => {
    const timeseries: TimeseriesPoint[] = [{ timestamp: FEB_1970 * 10, value: 10 }];
    const result = correctIncompleteTimeseries(
      timeseries,
      TemporalResolution.Monthly,
      TemporalResolutionOption.Year,
  ...
  it('returns OutOfScopeData for different months', () => {
    const timeseries: TimeseriesPoint[] = [{ timestamp: FEB_1970, value: 10 }];
    const result = correctIncompleteTimeseries(
      timeseries,
      TemporalResolution.Monthly,
  ...
  it('does not modify input', () => {
    const timeseries: TimeseriesPoint[] = [{ timestamp: 0, value: 10 }];
    const copy: TimeseriesPoint[] = [{ timestamp: 0, value: 10 }];
    const result = correctIncompleteTimeseries(
      timeseries,
      TemporalResolution.Monthly,
      TemporalResolutionOption.Year,
  ...
  it('extrapolates daily to month', () => {
    ...
    const result = correctIncompleteTimeseries(
      ...
      TemporalResolutionOption.Month,
      AggregationOption.Sum,
      new Date(APR_1970 + addDays * ONE_DAY)
    );

    expect(result.action).to.equal(IncompleteDataCorrectiveAction.DataExtrapolated);
    expect(result.points.length).to.equal(2);
    expect(result.points[0]).to.deep.equal(timeseries[0]);
    expect(result.points[1].value).to.equal(timeseries[1].value * (30 / (1 + addDays))); // +1 since we started on the first of the month
  });
  ...
  it('extrapolates weekly to month', () => {
    ...
    const result = correctIncompleteTimeseries(
      ...
      TemporalResolutionOption.Month,
      AggregationOption.Sum,
      new Date(NOV_1970 + addDays * ONE_DAY)
    );

    expect(result.action).to.equal(IncompleteDataCorrectiveAction.DataExtrapolated);
    expect(result.points.length).to.equal(2);
    expect(result.points[0]).to.deep.equal(timeseries[0]);
    expect(result.points[1].value).to.equal(timeseries[1].value * (4 / ((1 + addDays) / 7))); // +1 since we started on the first of the month
  });
  ...
  it('extrapolates dekad to month', () => {
    ...
    const result = correctIncompleteTimeseries(
      ...
      TemporalResolutionOption.Month,
      AggregationOption.Sum,
      new Date(OCT_1970 + addDays * ONE_DAY)
    );

    expect(result.action).to.equal(IncompleteDataCorrectiveAction.DataExtrapolated);
    expect(result.points.length).to.equal(2);
    expect(result.points[0]).to.deep.equal(timeseries[0]);
    expect(result.points[1].value).to.equal(timeseries[1].value * (3 / ((1 + addDays) / 10))); // +1 since we started on the first of the month
  });
  ...
  it('extrapolates daily to year', () => {
    ...
    const result = correctIncompleteTimeseries(
      ...
      TemporalResolutionOption.Year,
      AggregationOption.Sum,
      new Date(dayNum * ONE_DAY)
    );

    expect(result.action).to.equal(IncompleteDataCorrectiveAction.DataExtrapolated);
    expect(result.points.length).to.equal(1);
    expect(result.points[0].value).to.be.approximately(
      timeseries[0].value * (365 / dayNum),
      0.0001
    );
  });
  ...
  it('extrapolates weekly to year', () => {
    ...
    expect(result.points.length).to.equal(1);
    expect(result.points[0].value).to.be.approximately(
      timeseries[0].value * (52 / (dayNum / 7)),
      0.0001
    );
  });
  ...
  it('extrapolates dekad to year', () => {
    ...
    expect(result.points.length).to.equal(1);
    expect(result.points[0].value).to.be.approximately(
      timeseries[0].value * (36 / (dayNum / 10)),
      0.0001
    );
  });
