 ...
 */
export const setProcessingSucceeded = async (metadata: any) => {
  ...
  Logger.info(
    `Marking pipeline job successful data_id=${dataId}, run_id=${runId}, flow_id=${flowId}`
  );
  ...
  const updateDelta = docIds.map((docId: string) => {
    return {
      id: docId,
      status: 'READY',
      flow_id: flowId,
 ...
 */
export const setProcessingFailed = async (metadata: any) => {
  const dataId = metadata.data_id;
  const runId = metadata.run_id;
  const docIds = metadata.doc_ids;
  const isIndicator = metadata.is_indicator;
 ...
 */
export const setRuntimeQueued = async (metadata: any) => {
  const dataId = metadata.data_id;
  const runId = metadata.run_id;
  const docIds = metadata.doc_ids;
  const isIndicator = metadata.is_indicator;
 ...
 */
const updateDocuments = async (updateDelta: any[], isIndicator: boolean) => {
  if (isIndicator) {
 ...
 */
const fetchPipelineResults = async (dataId: string, runId: string) => {
  const options = {
    method: 'GET',
    url:
