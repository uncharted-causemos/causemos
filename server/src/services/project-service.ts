 ...
 * Returns immediately with the new index identifier.
 */
export const createProject = async (name: string, description: string) => {
  ...
  const promises = DEFAULT_QUESTIONS.map((question) =>
    questionService.createQuestion(question, projectId, 'private', [], null)
  );
 ...
 */
export const checkIndexStatus = async (projectId: string) => {
  const project = Adapter.get(RESOURCE.PROJECT);
