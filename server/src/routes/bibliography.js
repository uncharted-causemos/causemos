...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const cagId of ids) {
      ...
      // 0. Get CAG
      const model = await modelAdapter.findOne([{ field: 'id', value: cagId }], {});

      if (model) {
        // 1. Get edges
        const edges = await edgeAdapter.find([{ field: 'model_id', value: cagId }], { size: 5000 });
        ...
        const statements = await statementAdapter.find(
          {
            clauses: [{ field: 'id', values: statementIds, operand: 'OR', isNot: false }],
          },
