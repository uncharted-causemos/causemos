...
const getBaseTiles = async (res: Response, url: string) => {
  try {
    const response = await axios.get(url, { responseType: 'stream' });
    response.data.pipe(res).on('end', () => {
...
router.get(
  ...
  authUtil.checkRole([authUtil.ROLES.USER]),
  asyncHandler(async (req, res) => {
    const stylesheet = JSON.parse(JSON.stringify(baseMapStyle)) as any; // Deep clone json file to prevent from mutation
    /**
     * Notes:
