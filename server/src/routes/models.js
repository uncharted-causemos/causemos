...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const historyLogs = await historyConn.find([{ field: 'model_id', value: modelId }], {
      size: 10000,
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { name, description, thumbnail_source: thumbnailSource, data_analysis_id } = req.body;
...
const buildCreateModelPayload = async (modelId) => {
  ...
  const payload = await modelService.buildCreateModelPayload(
    model,
    modelComponents.nodes,
    modelComponents.edges
  );
  return payload;
};
...
const processInferredEdgeWeights = async (modelId, engine, inferredEdgeMap) => {
  ...
  for (const edgeParameter of components.edges) {
    ...
    Logger.debug(
      `${key} => engineInferred=${engineInferredWeights}, current=${currentWeights}, currentEngine=${currentEngineWeights}`
    );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (setLock(modelId) === false) {
      Logger.warn(
        `Conflict while registering model ${modelId} with ${engine}. Another transaction in progress`
      );
      res
        .status(409)
        .send(
          `Conflict while registering model ${modelId} with ${engine}. ` + TRANSACTION_LOCK_MSG
        );
      return;
    }
    ...
    if (edgesToOverride.length > 0) {
      await dyseService.updateEdgeParameter(
        modelId,
        modelService.buildEdgeParametersPayload(edgesToOverride)
      );
    }

    // 4. Update model status
    const status =
      initialParameters.status === 'training' ? MODEL_STATUS.TRAINING : MODEL_STATUS.READY;
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (setLock(modelId) === false) {
      Logger.info(
        `Conflict while updating model ${modelId} node-parameter. Another transaction in progress`
      );
      res
        .status(409)
        .send(`Conflict while updating node for model ${modelId}. ` + TRANSACTION_LOCK_MSG);
      return;
    }
    ...
    if (_.isNil(nodeParameter.parameter.min) || _.isNil(nodeParameter.parameter.max)) {
      ...
      const { min, max } = modelUtil.projectionValueRange(
        nodeParameter.parameter.timeseries.map((d) => d.value)
      );
    ...
    const getNodePayload = [{ field: 'id', value: nodeParameter.id }];
    ...
    if (
      ...
      (beforeIndicatorId === null || currentIndicatorId !== beforeIndicatorId)
    ) {
...
const clearEdgeWeightsForNode = async (modelId, nodeConcept) => {
  ...
  if (adjacentEdgesToUpdate.length > 0) {
    const results = await edgeParameterAdapter.update(adjacentEdgesToUpdate, (doc) => doc.id);
    if (results.errors) {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (setLock(modelId) === false) {
      Logger.info(
        `Conflict while updateing model ${modelId} edge-parameter. Another transaction in progress`
      );
      res
        .status(409)
        .send(`Conflict while updating edge for model ${modelId}. ` + TRANSACTION_LOCK_MSG);
      return;
    }

    const payload = modelService.buildEdgeParametersPayload([
      { source, target, polarity, parameter },
    ]);
